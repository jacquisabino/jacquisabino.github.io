...
1. Set up the input trace files in the desired format. Each line in the trace file represents a memory access. 
   Trace files, run.sh, mean_and_std.py and with_penalties.py must all be in the same directory. 
...
In run.sh replace the file in FILE_NAMES_TEST=("047.tomcatv.din") with any of the dinero file names. The output to formatted.txt will be a summary of cache hits and misses averaged over 10 iterations of that file for associativity of 2, 4 and 8. 
